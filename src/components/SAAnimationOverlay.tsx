...
export const SAAnimationOverlay = ({ budget, candidates, animationSteps, totalIterations, onFinish }: SAAnimationOverlayProps) => {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      const stepBasket = step.basketIds.map((id: number) => candidates.find((c) => c.appid === id)).filter(Boolean) as Candidate[];
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-3 md:gap-4">
              {[...Array(slotCount)].map((_, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`aspect-[3/4] rounded-2xl border-2 transition-all duration-300 overflow-hidden relative group ${item ? "border-orange-500/50 bg-zinc-900 shadow-[0_0_20px_rgba(249,115,22,0.1)]" : "border-white/5 bg-white/[0.02] border-dashed"}`}
                  >
                    {item ? (
                      <>
                        <img
            ...
            <div className="glass p-4 md:p-6 rounded-3xl border-white/10 space-y-4">
              <div className="flex flex-col sm:flex-row justify-between text-[10px] font-black uppercase tracking-widest gap-2">
                <span className="text-zinc-500">
                  Current Cost: <span className="text-white">Rp{totalCost.toLocaleString("id-ID")}</span>
                </span>
                <span className="text-zinc-500">
                  Target: <span className="text-orange-500">Rp{budget.toLocaleString("id-ID")}</span>
                </span>
              </div>
