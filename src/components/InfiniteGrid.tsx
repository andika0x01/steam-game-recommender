        ...
        {items.length > 0
          ? items.map((item, idx) => {
              ...
              return item.analyzerData ? (
                <div key={`${item.appid || item.appId}-${idx}`} className="cursor-pointer recommendation-card-trigger" data-game={JSON.stringify(item.analyzerData)}>
                  {card}
                </div>
              ) : (
                <React.Fragment key={`${item.appid || item.appId}-${idx}`}>{card}</React.Fragment>
              );
            })
          : [...Array(12)].map((_, i) => <div key={i} className="aspect-[3/4] bg-white/5 rounded-[2rem] animate-pulse"></div>)}
      </div>
      ...
      {hasMore && (
        <div ref={loaderRef} className="py-20 flex flex-col items-center gap-8">
          {loading ? (
            ...
            >
              <span className="text-xs font-black uppercase tracking-[0.3em] text-zinc-400 group-hover:text-white transition-colors">Load More Results</span>
              <svg
