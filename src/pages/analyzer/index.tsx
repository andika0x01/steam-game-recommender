...
app.get("/", async (c) => {
  ...
  const topTagLatexRows = topTags.map(([tag, score], index) => `W_{\\mathrm{${latexName(tag)}}} &= ${Number(score).toFixed(3)}\\quad\\text{rank ${index + 1}}`).join("\\\\");
  const topPublisherLatexRows = topPublishers
    .map(
      ([publisher, score], index) =>
        `P_{\\mathrm{${latexName(publisher)}}} &= ${Number(score).toFixed(3)}\\quad (${Math.round(Number(score) * 100)}\\%)\\quad\\text{rank ${index + 1}}`
    )
    .join("\\\\");
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-10">
          <div className="space-y-6 max-w-3xl">
            ...
            <h2 className="text-5xl md:text-8xl font-black tracking-tighter uppercase leading-[0.85]">
              Library <br />
              <span className="text-white/20 outline-text">Analysis</span>
            </h2>
...
\\]`}
              ...
              <p>
                Setiap baris adalah hasil agregasi bobot tag yang tampil di kartu. Nilai ini bukan persentase; tag makin besar jika sering muncul pada game library yang skor
                fuzzy-nya tinggi.
              </p>
            </div>
